...
//Присвоение 33 клавишам PLU номера
for ($i = 1; $i <= 33; $i++)
   {
     $formats[]=array(
     'number'=>$i, //номер клавиши
    'code'=>($i+3), 
    'type'=>0,
    );
   }
...
foreach($formats as $fr){
 $n++;
  $result[$n].=pack('i', $fr['number']); //номер клавиши
  $result[$n].=pack('s', 6); //длина записи
  
  $result[$n].=pack('i', $fr['code']); //номер клавиши
  $result[$n].=pack('c', $fr['type']); //тип
  $result[$n].="\x00"; //резерв
...
$data=$HEADER; // Заголовочная последовательность
 ...
 $body.=$FILE_TYPE_FUNC; //Тип файла PLU товары
